...
#' updates by SM:
#' 09/28/2021: Removed aggregation, makes output 500 m x 500 m resolution
#' 11/10/2022: Added impervious surface adjustment to heterotrophic respiration
#' 03/13/2024: Added impervious surface adjustment to autotrophic respiration
...
predReco_biome <- function(reg.name = 'westernCONUS', 
  ...
  try({
    ...
    # outpath for Reco
    if (isa.tf == T){
      reco.path <- file.path(reg.path, paste0('daily_mean_Reco_ISA_a_', nn.platform), nn.pattern, yr)
    }else{
      reco.path <- file.path(reg.path, paste0('daily_mean_Reco_no_ISA_', nn.platform), nn.pattern,yr)
    }
    ...
    # SM, added impervious surface adjustment 11/10/2022
    if (isa.tf == T){
      ...
      #UNCOMMENT BELOW TO USE Ra adjustment
      evi.ref <- prep.list$evi[EVI_index]
    
      ##Assuming no autotrophic respiration (barren)
      #Ra<-Ra*0
      #Ra_err<- Ra_err*0
    ...
    if (isa.tf == T){
      reco.fn <- file.path(reco.path, paste0('daily_mean_Reco_uncert_GMIS_',
                                             reg.name, '_', substr(timestr, 1, 8), '.nc'))
    }else{
      reco.fn <- file.path(reco.path, paste0('daily_mean_Reco_uncert_no_ISA_',
                                             reg.name, '_', substr(timestr, 1, 8), '.nc'))
    }
