...
compute.reco.hrly <- function(EVI, LSWI, WScale, TA.path, TA.varname, lc.path, lc.pattern, timestr, site.ext, proj.rt){
  ...
  setwd('C:/Users/kitty/Documents/Research/SIF/UrbanVPRM/UrbanVPRM/dataverse_files/MODIS_reflectance/MODIS_GTA_AppEEARS_2020') # landsat data in /urbanVPRM_30m/driver_data/landsat/
  ...
  qc_files <- mod_files[grep('qc',mod_files)]
  ...
  EVI_raster <- brick(ncol=624, nrow=552, xmn=-80.9, xmx=-78.3, ymn=42.4, ymx=44.7,nl=366)
  res(EVI_raster)
  ##check the number of cells is 344448 for the GTA
  ncell(EVI_raster)
  names(EVI_raster)<-paste('DOY',1:366,sep="") #name the layers
  ...
  for (i in 1:length(b01_files)){
    ...
    qc_file <- stack(qc_files[i+2])
    file1<-raster::projectRaster(b01_file, mean.reco.rt)#proj.rt)
    file2<-raster::projectRaster(b02_file, mean.reco.rt)#proj.rt)
    file3<-raster::projectRaster(b03_file, mean.reco.rt)# proj.rt)
    qcfile <- raster::projectRaster(qc_file,mean.reco.rt)
...
#    #assign(paste0('EVI_LSWI',i,'.dt'), merge(EVI.dt,LSWI.dt,by=c("Index","x","y")))
  ...
  #set the number of columns, rows, and extent NEED TO CHANGE DEPENDING ON SITE
  Inter_EVI_raster <- brick(ncol=624, nrow=552, xmn=-80.9, xmx=-78.3, ymn=42.4, ymx=44.7,nl=366)
  res(Inter_EVI_raster)
  #check the number of cells is 344448 for the GTA
  ncell(Inter_EVI_raster)
  names(Inter_EVI_raster)<-paste('DOY',1:366,sep="") #name the layers
  
  EVI_values<-values(EVI_raster)
  EVI_length<-length(values(EVI_raster$DOY1))
  Inter_EVI_values<-values(Inter_EVI_raster)
  
  for(i in 1:length(values(EVI_raster$DOY1))){
...
#  
#  #THIS WORKS!!! :D
  writeRaster(Inter_EVI_raster,filename="C:/Users/kitty/Documents/Research/SIF/SMUrF/data/MODIS_EVI_2020_qc_test.tif",
              overwrite=TRUE)
