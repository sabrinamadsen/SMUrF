...
indx <- 1
site <- 'GTA'
        #c('Guangzhou', 'Shanghai',   'Beijing',       'Wuhan',        'Taipei', 
...
reg.name <- 'easternCONUS'
            #c(rep('easternChina', 5), rep('easternCONUS', 5), 
            #  rep('westernCONUS', 5), rep('westernEurope', 5))[indx]  

# each processor works on a day in a year 
min.datestr <- '2018-01-01'
max.datestr <- '2018-12-31'
...
output.path <- file.path(homedir, 'SMUrF/output2018_3')
